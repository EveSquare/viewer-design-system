...
class BuildingsLayer {
  ...
  getLayer(step: number, simulation: Simulation): Promise<any> {
            ...
            .map((buildingEntity) => {
              ...
              if (edgesProps.isDefined) {
                ...
                if (buildingEntity.urn === URN_MAP["REFUGE"]) {
                  buildingObject.floors = this.getValue(
                    buildingEntity,
                    "FLOORS"
                  );
                  buildingObject.capacity = this.getValue(
                    buildingEntity,
                    "CAPACITY"
                  );
                  buildingObject.importance = this.getValue(
                    buildingEntity,
                    "IMPORTANCE"
                  );
                  buildingObject.capacity = this.getValue(
                  ...
                  buildingObject.bedCapacity = this.getValue(
                    buildingEntity,
                    "BED_CAPACITY"
                  );
                  buildingObject.occupiedBeds = this.getValue(
                    buildingEntity,
                    "OCCUPIED_BEDS"
                  );
                  buildingObject.refillCapacity = this.getValue(
                    buildingEntity,
                    "REFILL_CAPACITY"
                  );
                  buildingObject.waitingListSize = this.getValue(
                    buildingEntity,
                    "WAITING_LIST_SIZE"
                  );
                }
